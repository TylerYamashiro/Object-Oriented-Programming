 ...
 */
  ...
  */
   ...
   *The class also has build in operator overloading only between
   *other numMixer objects in order to prevent mixed mode operations
   *and to enforce encapuslation.  The +, +=, - , -=, =, ==, and != operations
...
numMixer numMixer::operator+(const numMixer& b){
  numMixer local(b);
  for(int i = 0; i < STORELENGTH; i++){
    local.store[i] += store[i];
  }
  return local;
}
...
//is the sum of both elements in the argument pools.
numMixer& numMixer::operator+=(numMixer& b){
  for(int i = 0; i < STORELENGTH; i++){
    store[i] += b.store[i];
  }
  return *this;
...
bool numMixer::operator==(const numMixer& b){
  for(int i = 0; i < STORELENGTH; i++){
    if(store[i] != b.store[i])
      return false;
  }
  return true;
}
